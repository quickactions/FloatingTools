...
public sealed class DirectionalTextBoxRuntimeTests
{
    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public void LongWrappedTextAnchorsItsLastLineOnThePhysicalLanguageSide(bool hebrew)
        => WpfTestApplication.Run(() =>
        {
            var textBox = new DirectionalTextBox
            {
                ...
                Text = hebrew ? HebrewWrappingText : EnglishWrappingText
            };
    ...
    private static LastLineInk GetLastLineInk(DirectionalTextBox textBox)
    {
